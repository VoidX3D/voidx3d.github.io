...
export default function ProjectGrid({ repos, featured, languages }: Props) {
  ...
  useEffect(() => {
    if (headerRef.current) {
      animate(headerRef.current.querySelectorAll('.filter-btn'), {
        opacity: [0, 1],
        translateY: [8, 0],
        delay: stagger(0.04),
        duration: 400,
    ...
    <section id="projects" className="py-20">
      <div className="max-w-5xl mx-auto px-4">
        {featured.length > 0 && (
          <>
            <motion.h2
              className="text-xs font-semibold text-white/40 tracking-[0.2em] uppercase mb-6"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-16">
              {featured.map((repo, i) => (
                <ProjectCard key={repo.id} repo={repo} featured index={i} />
              ))}
            </div>
          </>
        )}
        ...
        <div className="flex items-center justify-between gap-4 mb-5" ref={headerRef}>
          <motion.h2
            className="text-xs font-semibold text-white/40 tracking-[0.2em] uppercase"
          >
            Projects <span className="text-white/10 font-normal tracking-normal">({repos.length})</span>
          </motion.h2>
        ...
        {filtered.length === 0 && (
          <p className="text-white/15 text-xs text-center py-10">No projects for this language.</p>
        )}
      </div>
    </section>
  )
