...
export default function ProjectCard({ repo, featured = false, index }: Props) {
  ...
  return (
    <motion.a
      ...
      className="card"
      style={featured ? { padding: '20px' } : { padding: '14px' }}
        ...
        <>
          <div className="flex items-start justify-between gap-2 mb-2.5">
            ...
            {repo.stargazers_count > 0 && (
              <span className="shrink-0 flex items-center gap-1 text-[11px]" style={{ color: 'var(--text-tertiary)' }}>
          ...
          {repo.description && <p className="text-xs leading-relaxed line-clamp-2 mb-3" style={{ color: 'var(--text-secondary)' }}>{repo.description}</p>}
          <div className="flex items-center gap-2.5 flex-wrap">
            ...
            {repo.topics?.slice(0, 2).map(t => (
              <span key={t} className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: 'var(--bg-elevated)', color: 'var(--text-quaternary)' }}>{t}</span>
            ))}
          </div>
        </>
      )}
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            {repo.language && (
              <span className="flex items-center gap-1 shrink-0">
                ...
                <span className="text-[10px]" style={{ color: 'var(--text-quaternary)' }}>{repo.language}</span>
              </span>
            )}
          </div>
          {repo.description && <p className="text-xs leading-relaxed line-clamp-1" style={{ color: 'var(--text-secondary)' }}>{repo.description}</p>}
          <div className="flex items-center gap-2.5 mt-1.5 text-[10px]" style={{ color: 'var(--text-quaternary)' }}>
